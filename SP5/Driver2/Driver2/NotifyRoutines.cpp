...
NTSTATUS OnRegistryNotify(PVOID context, PVOID arg1, PVOID arg2) {
    ...
    if (!NT_SUCCESS(status)) {
        KdPrint(("Failed to open file with status: 0x%X\n", status));
        return STATUS_SUCCESS;
    }

    REG_SET_VALUE_KEY_INFORMATION* vkInfo = (REG_SET_VALUE_KEY_INFORMATION*)info->PreInformation;
    if (vkInfo == NULL) {
        KdPrint(("No pre-information available. Ignoring\n"));
        ZwClose(hFile);
        return STATUS_SUCCESS;
    }
    ...
    if (!NT_SUCCESS(status)) {
        KdPrint(("Failed to write to file with status: 0x%X\n", status));
        ZwClose(hFile);
        return STATUS_SUCCESS;
    }
    ...
    KdPrint(("Changes logged successfully\n"));
