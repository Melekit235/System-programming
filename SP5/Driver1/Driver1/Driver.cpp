...
extern "C" NTSTATUS DriverEntry(_In_ PDRIVER_OBJECT  DriverObject, _In_ PUNICODE_STRING RegistryPath) {
	...
	UNICODE_STRING symLink = RTL_CONSTANT_STRING(L"\\DosDevices\\ProcessesLifeCycleWatcher");
...
void ProcessesLifeCycleWatcherUnload(_In_ PDRIVER_OBJECT DriverObject) {
	KdPrint(("ProcessesLifeCycleWatcherUnload"));
	UNICODE_STRING symLink = RTL_CONSTANT_STRING(L"\\DosDevices\\ProcessesLifeCycleWatcher");
...
NTSTATUS ProcessesLifeCycleWatcherCreateClose(_In_ PDEVICE_OBJECT DeviceObject, _In_ PIRP Irp) {
	KdPrint(("ProcessesLifeCycleWatcherCreateClose"));
...
void OnProcessNotify(PEPROCESS Process, HANDLE ProcessId, PPS_CREATE_NOTIFY_INFO CreateInfo) {
	KdPrint(("OnProcessNotify: ProcessId=%lu\n", HandleToULong(ProcessId)));
	...
	if (CreateInfo != NULL) {
		...
		if ((CreateInfo->ImageFileName == NULL) ||
			(RtlCompareUnicodeString(&targetImage, CreateInfo->ImageFileName, TRUE) != 0) &&
			(RtlCompareUnicodeString(&targetImage, CreateInfo->ImageFileName, FALSE) != 0)) {
			KdPrint(("Target process not created. ImageFileName: %wZ Expected: %wZ\n", CreateInfo->ImageFileName, targetImage));
			return;
		}
		KdPrint(("Target process created"));
		ProcessEventItem* eItem = (ProcessEventItem*)ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(ProcessEventItem), DRIVER_TAG);
		if (eItem == NULL) {
			KdPrint(("Failed to allocate pool 1"));
			return;
		}
		KdPrint(("Allocate pool"));
		...
		ProcessEventItem* eItem = (ProcessEventItem*)ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(ProcessEventItem), DRIVER_TAG);
		if (eItem == NULL) {
			KdPrint(("Failed to allocate pool 2"));
			return;
		}
...
NTSTATUS ProcessesLifeCycleWatcherRead(_In_ PDEVICE_OBJECT DeviceObject, _In_ PIRP Irp) {
	KdPrint(("ProcessesLifeCycleWatcherRead"));
	UNREFERENCED_PARAMETER(DeviceObject);
